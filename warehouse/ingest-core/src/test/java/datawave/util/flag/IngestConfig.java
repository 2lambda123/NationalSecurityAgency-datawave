...
/**
 * Contains configuration parameters for Flag Maker testing. The class functions
 * as a singleton. Any configuration or initialization errors will cause the
 * class to go into an illegal state condition.
 */
class IngestConfig {
  private static final Logger logger =
      LoggerFactory.getLogger(IngestConfig.class);
  final static Gson gson = new Gson();

  private static final AtomicReference<IngestConfig> cfg =
      new AtomicReference<>();
  ...
  static void create(final String fName) throws IOException {
    ...
    @SuppressWarnings("deprecation") final JsonParser parser = new JsonParser();
    try (final Reader rdr = new FileReader(fName)) {
      @SuppressWarnings("deprecation")
      final JsonElement json = parser.parse(rdr);
      final IngestConfig val =
          gson.fromJson(json.toString(), IngestConfig.class);
      cfg.set(val);
  ...
  static IngestConfig getInstance() {
    ...
    if (null == val) {
      throw new IllegalStateException(
          "ingest configuration has not been initialized");
    }
    return val;
  }

  FileSystem getHdfs() { return this.fs; }

  String getHdfsIngestDir() { return this.hdfsIngestDir; }

  int getWorkers() { return this.workers; }
   ...
   */
  int getDurtion() { return this.duration * 1000 * 60; }

  /**
   * Returns a random interval based upon the minimum and maximum interval
   * settings.
   *
  ...
  /**
   * Returns a random list of files to ingest based upon the minimum and maximum
   * ingest file settings.
   *
  ...
  /**
   * Creates a {@link FileSystem} object based upon the hdfs configuration
   * property.
   *
