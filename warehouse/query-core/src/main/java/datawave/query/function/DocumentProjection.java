...
/**
 * Applies an includes or excludes projection to a Document. Includes projection
 * will preserve Document sub-substructure whereas excludes projection will
 * prune sub-substructure which does not match the excludes. <p> e.g. Input:
 * {NAME:'bob', CHILDREN:[{NAME:'frank', AGE:12}, {NAME:'sally', AGE:10}],
 * AGE:40} <p> Include of 'NAME' applied: {NAME:'bob', CHILDREN:[{NAME:'frank'},
 * {NAME:'sally'}]} <p> Exclude of 'NAME' applied: {CHILDREN:[{AGE:12},
 * {AGE:10}], AGE:40}
 */
public class DocumentProjection implements DocumentPermutation {
  ...
  public DocumentProjection(boolean includeGroupingContext,
                            boolean reducedResponse, boolean trackSizes,
  ...
  /*
   * This contstructor will not include the grouping context and will not reduce
   * the document
   */
  @Deprecated
  public DocumentProjection(Set<String> projections,
                            Projection.ProjectionType projectionType) {
  ...
  @Deprecated
  public DocumentProjection(boolean includeGroupingContext,
                            boolean reducedResponse, Set<String> projections,
                            Projection.ProjectionType projectionType) {
    this(includeGroupingContext, reducedResponse, true, projections,
         projectionType);
  }

  public DocumentProjection(boolean includeGroupingContext,
                            boolean isReducedResponse, boolean isTrackSizes,
  ...
  private Document trim(Document d) {
    ...
    for (Entry<String, Attribute<? extends Comparable<?>>> entry :
         dict.entrySet()) {
      ...
      if (projection.apply(fieldName)) {
        ...
        // If the projection is configured to exclude, we must fully traverse
        // the subtree
        if (projection.isUseExcludes()) {
          if (attr instanceof Document) {
            ...
            if (0 < newSubDoc.size()) {
              newDoc.put(fieldName, newSubDoc.copy(),
                         this.includeGroupingContext, this.reducedResponse);
            }
            ...
            if (0 < subAttrs.size()) {
              newDoc.put(fieldName, subAttrs.copy(),
                         this.includeGroupingContext, this.reducedResponse);
            }
        ...
        newDoc.put(fieldName, (Attribute<?>)attr.copy(),
                   this.includeGroupingContext, this.reducedResponse);
        ...
        if (attr instanceof Document) {
          ...
          if (0 < newSubDoc.size()) {
            newDoc.put(fieldName, newSubDoc.copy(), this.includeGroupingContext,
                       this.reducedResponse);
          }
        } else if (attr instanceof Attributes) {
          // Since Document instances can be nested under attributes and
          // vice-versa all the way down, we need to pass along the fieldName so
          // that when we have come up with a nested document it can be
          // evaluated by its own name
          Attributes subAttrs = trim((Attributes)attr, fieldName);
