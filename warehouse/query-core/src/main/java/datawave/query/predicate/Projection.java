...
/**
 * Predicate that either retains attributes in a specified set of include fields
 * or removes attributes specified in a set of exclude fields. <p> This class is
 * <b>not thread safe</b>
 */
public final class Projection implements Predicate<String> {
  ...
  public Projection(@Nonnull Set<String> items, @Nonnull ProjectionType type) {
    ...
    if (type == ProjectionType.INCLUDES) {
      // do not make a copy of the incoming include fields. It could be a
      // UniversalSet
  ...
  public boolean isUseIncludes() { return type == ProjectionType.INCLUDES; }

  public boolean isUseExcludes() { return type == ProjectionType.EXCLUDES; }

  /**
   * Applies this projection to a field name
   *
   * @param inputFieldName
   *            an input field name, possibly with a grouping context or
   * identifier prefix
  ...
  public boolean apply(String inputFieldName) {
    if (!this.initialized) {
      throw new RuntimeException("This Projection must be initialized with a " +
                                 "set of includes or excludes fields");
    }
    String fieldName =
        JexlASTHelper.deconstructIdentifier(inputFieldName, false);
