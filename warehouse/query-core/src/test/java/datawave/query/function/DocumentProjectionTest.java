...
public class DocumentProjectionTest {
  ...
  public void setup() {
    d = new Document();
    d.put("FOO", new Content("foofighter",
                             new Key("row", "dt\0uid", "", cv, -1), true));
    d.put("ID", new Numeric(123, new Key("row", "dt\0uid", "", cv, -1), true));

    Document primes = new Document();
    primes.put("PRIME",
               new Numeric(2, new Key("row", "dt\0uid", "", cv, -1), true));
    primes.put("PRIME",
               new Numeric(3, new Key("row", "dt\0uid", "", cv, -1), true));
    primes.put("PRIME",
               new Numeric(5, new Key("row", "dt\0uid", "", cv, -1), true));
    primes.put("PRIME",
               new Numeric(7, new Key("row", "dt\0uid", "", cv, -1), true));
    primes.put("PRIME",
               new Numeric(11, new Key("row", "dt\0uid", "", cv, -1), true));
    d.put("PRIMES", primes);
    ...
    sub1.put("FOO.1",
             new Content("bar", new Key("row", "dt\0uid", "", cv, -1), true));
    sub1.put("ID.1",
             new Numeric(456, new Key("row", "dt\0uid", "", cv, -1), true));
    others.add(sub1);

    Document sub2 = new Document();
    sub2.put("FOO.2",
             new Content("baz", new Key("row", "dt\0uid", "", cv, -1), true));
    sub2.put("ID.2",
             new Numeric(789, new Key("row", "dt\0uid", "", cv, -1), true));
    others.add(sub2);
  ...
  public void testIncludesSingleFieldDeprecated() {
    ...
    DocumentProjection projection = new DocumentProjection(
        false, false, true, includes, Projection.ProjectionType.INCLUDES);

    assertEquals(11, d.size());
    Map.Entry<Key, Document> result =
        projection.apply(Maps.immutableEntry(new Key(), d));
  ...
  public void testIncludesNoFieldsSpecifiedDeprecated() {
    DocumentProjection projection =
        new DocumentProjection(false, false, true, Collections.emptySet(),
  ...
  public void testIncludesAllFieldsDeprecated() {
    Set<String> includes =
        Sets.newHashSet("FOO", "ID", "PRIMES", "PRIME", "CHILDREN");
    DocumentProjection projection = new DocumentProjection(
        false, false, true, includes, Projection.ProjectionType.INCLUDES);
  ...
  // even though the sub-document field is not on the includes list, all the
  // fields in the sub-document are. Therefore, we keep the child document.
  @Test
  ...
  public void testExcludeSingleFieldDeprecated() {
    ...
    DocumentProjection projection = new DocumentProjection(
        false, false, true, excludes, Projection.ProjectionType.EXCLUDES);
  ...
  public void testConfirmFieldExcludedDeprecated() {
    ...
    assertEquals(6, result.getValue().size());
    assertFalse(
        result.getValue().containsKey("PRIMES")); // key no longer exists
  }
  ...
  public void testIncludesExampleCaseDepricated() {
    ...
    DocumentProjection projection =
        new DocumentProjection(false, false, true, Sets.newHashSet("NAME"),
                               Projection.ProjectionType.INCLUDES);
  ...
  private Document buildExampleDocument() {
    ...
    d.put("NAME",
          new Content("bob", new Key("row", "dt\0uid", "", cv, -1), true));
    d.put("AGE", new Numeric(40, new Key("row", "dt\0uid", "", cv, -1), true));

    Attributes children = new Attributes(true);

    Document frank = new Document();
    frank.put("NAME", new Content("frank",
                                  new Key("row", "dt\0uid", "", cv, -1), true));
    frank.put("AGE",
              new Numeric(12, new Key("row", "dt\0uid", "", cv, -1), true));
    children.add(frank);

    Document sally = new Document();
    sally.put("NAME", new Content("sally",
                                  new Key("row", "dt\0uid", "", cv, -1), true));
    sally.put("AGE",
              new Numeric(10, new Key("row", "dt\0uid", "", cv, -1), true));
    children.add(sally);
